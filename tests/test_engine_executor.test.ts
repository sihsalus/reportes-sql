...
describe("executeAndPersist", () => {
  ...
  test("supersedes canonical rows across versions when indicadorId is provided", async () => {
    ...
    const crossVersionSql = mockSequelizeQuery.mock.calls[2]?.[0] as string;
    ...
    expect(mockSequelizeQuery.mock.calls[1]?.[0]).toContain("FOR UPDATE OF i");
    expect(mockSequelizeQuery.mock.calls[1]?.[1]).toEqual(expect.objectContaining({ transaction: mockTransaction }));
