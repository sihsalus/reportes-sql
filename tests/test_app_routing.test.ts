...
const MOCKED_SESSION_USER = {
  ...
  privileges: [
    { display: "app:indicadores", uuid: "priv-read" },
...
describe("incoming auth — requireSession gate", () => {
  ...
  it("returns 403 fail-closed on every write/recalc handler (privilege unset)", async () => {
    const { request } = requestFor("");
  ...
  it.each(["/indicadores", "/resultados", "/metas", "/conceptos/encounter-types"])(
    "requires the canonical indicators privilege before reading %s",
    async (path) => {
      const { request } = requestFor(prefix);
      mockFetch.mockResolvedValueOnce({
        ok: true,
        status: 200,
        json: async () => ({ authenticated: true, user: { uuid: "other-user", privileges: [] } }),
      } as Response);

      const response = await request.get(`${prefix}${path}`).set(...session);

      expect(response.status).toBe(403);
      expect(response.body).toEqual({ detail: "Sin privilegios" });
