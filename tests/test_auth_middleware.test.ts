...
describe("requireSession", () => {
  ...
  test.each([undefined, null, {}, { uuid: "" }])("rejects an authenticated response with invalid user %p", async (user) => {
    (globalThis.fetch as jest.Mock).mockResolvedValue(
      mockFetchRes(200, { authenticated: true, user }),
    );
    const req = makeReq({ headers: { cookie: "JSESSIONID=synthetic-session" } });
    const res = makeRes();
    const next = makeNext();
