...
export async function executeAndPersist(
  ...
  try {
    ...
    if (results.length > 0) {
      ...
      try {
        if (mesReferencia) {
          // Lock the actual parent indicator, including callers without an
          // indicadorId. Concurrent calculations cannot insert two canonical
          // rows after both observed the previous canonical row.
          await sequelize.query(
            `SELECT i.id FROM indicador i
             JOIN indicador_version iv ON iv.indicador_id = i.id
             WHERE iv.id = :version_id FOR UPDATE OF i`,
            { replacements: { version_id: indicadorVersionId }, transaction: tx, type: QueryTypes.SELECT },
          );
          if (opts.indicadorId) {
